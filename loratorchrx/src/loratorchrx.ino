  ...
  if (manager.available()) {
    ...
    if (manager.recvfromAck(buf, &len, &from)) {
      ...
      if (strcmp((char *)buf, "on") == 0) {
        ...
        digitalWrite(LAMP_OUT_PIN, 1);
      }
    }
  }

  // // detect lamp on button (only valid when lamp is remote controlled off - that is LAMP_OUT_PIN set to 1)
  // lampOnButton.read();
